...
void playerLogin(){
    ...
    if(choice == 'y' || choice == 'Y'){
        FILE *f = fopen("../test/savedata.txt", "r");
        if(f == NULL){
...
void listRedeem(){
    ...
    Redeem redeems[REDEEM] = {
        ...
        {"3. Double Poin untuk pertanyaan berikutnya -- ", "Poin pertanyaan berikutnya akan dikalikan 2 (hanya 1 pertanyaan)\n", 80}
    };
    for(int i = 0; i < 3; i++){
...
void showLeaderboard() {
    ...
    FILE *write = fopen("../test/leaderboard.txt", "w");
    if (write == NULL) {
